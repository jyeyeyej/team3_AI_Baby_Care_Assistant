...
> 구현 정합성 안내: 이 문서는 현재 MVP 구현 범위와 향후 확장할 Agent 구조를 함께 설명합니다.현재 MVP에서는 권한, 승인, 기록 저장 등 중요한 기능을 Backend에서 관리하며, AI는 사용자의 요청을 이해하고 육아 정보를 안내하는 데 활용합니다. 
...
일반 육아 질문의 RAG 카테고리는 특정 단어 목록만으로 결정하지 않습니다. Backend가
구조화된 LLM 분류 결과를 받아 `feeding`, `sleep`, `weaning`, `development`, `safety`,
`general_baby`, `out_of_scope` 중 하나로 검증합니다. 전용 주제는 해당 RAG Tool을
선택하고, 아기 목욕·외출·놀이처럼 전용 주제에 맞지 않는 육아 질문은 `general_baby`로
분류해 안전한 일반 육아 안내를 반환합니다. 예를 들어 “자다가 우는데 왜 그래?”는
`sleep`으로 분류합니다.

기록 저장·수정처럼 DB 상태를 바꾸는 요청은 LLM 분류 결과만으로 실행하지 않습니다.
수유량·수유 방식·입력 출처 등은 기존의 결정론적 검증과 승인 정책을 통과해야 합니다.
분류 모델을 사용할 수 없거나 유효하지 않은 구조화 결과를 반환하면 RAG Tool을
추측해서 호출하지 않고 범위 안내를 반환합니다. 따라서 일반 질문 카테고리를
키워드 목록으로 판정하지 않습니다.

RAG 검색 결과에 출처가 없고 신뢰도가 `low`이거나 `general_baby`로 분류되면,
Backend는 일반 육아 안내를 생성할 수 있습니다. 이 안내는 질병 진단·약 용량·처방을
제시하지 않으며, 월령·경과·동반 증상을 추가로 확인하고 위험 신호가 있으면 진료·119
안내를 우선합니다.
